...
- Code: Phases 0–4 on `main`; visual shell pass in progress
...
| **2** | MachineRegistry, SubsynthMachine, rack, macros |
| **3** | DAW shell: transport, tabs, header, picker, ViewModel |
| **4** | Perform / Shape / Write host, param catalog, Write stub |

Installable debug APK via GitHub Actions artifact.

---

## Visual shell pass (current)

Align chrome with `docs/ui-kit/` before more DSP.
...
| **V7** | IBM Plex font assets (or downloadable fonts) | 00 |

**Exit criteria:** empty state + one Subsynth tab look recognizably like kit screens 01–02.

---

## Product roadmap (after visual shell)

### Milestone A — Playable shell
- Pattern clock native (SSD §7)
- Write: 16-step → engine; transport BBT from playhead
- Melodic keyboard (note on/off velocity)
